...
(function() {
    ...
    function createUI() {
        ...
        ui.id = 'target-bot-ui';
        ui.innerHTML = `
            <div style="position: fixed; bottom: 20px; right: 20px; background: #fff; border: 2px solid #cc0000; padding: 15px; z-index: 999999; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); font-family: Arial, sans-serif; width: 250px;">
                <h3 style="margin: 0 0 10px 0; color: #cc0000; text-align: center; font-size: 16px;">Target Auto Buyer</h3>
                
                <div style="margin-bottom: 10px;">
                    <label style="font-size: 12px; display: block;">Password:</label>
                    <input type="password" id="bot-password" value="${GM_getValue('password', 'Hacktanha')}" style="width: 100%; box-sizing: border-box; padding: 5px; margin-top: 3px;" />
                </div>
        ...
        document.getElementById('bot-start').addEventListener('click', () => {
            GM_setValue('password', document.getElementById('bot-password').value);
    ...
    function handleProductPage() {
        ...
        const addToCartBtn = getElementByXpath("//button[contains(text(),'Add to cart')]") ||
        ...
        if (addToCartBtn) {
            ...
            console.log("Target Bot: Add to cart not found. Refreshing in 3-5 seconds...");
